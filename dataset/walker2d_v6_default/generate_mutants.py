...
variation = 0.0 # range of random variation.
fixed_distance_to_ground = 0.1

# expected length and weight: mean
original_limb_length = np.array([0.4, 0.45, 0.5, 0.2])
original_limb_weight = np.array([0.05, 0.05, 0.04, 0.06])

file_list = []
param_list = []
power_coef_list = []
for mutant_id in range(1):
    ...
    data = {
        ...
        "length_torso": limb_length[0],
        "length_thigh": limb_length[1],
        "length_leg": limb_length[2],
        "length_foot": limb_length[3],
    }
    # Take .04f
    for key in data:
