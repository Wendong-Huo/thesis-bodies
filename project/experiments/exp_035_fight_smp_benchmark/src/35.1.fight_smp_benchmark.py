...
venv = DummyVecEnv([make_env(robot_bodies[i % len(robot_bodies)], rank=i, visualize_rank=-1) for i in range(12)])
normalize_kwargs = {}
...
for test_body in robot_bodies+zero_shot_bodies:
    ...
    # else:
    eval_venv = DummyVecEnv([make_env(test_body, rank=1)]) # 1 avoid visualization
...
model = PPO("MlpPolicy", env=venv, verbose=1, tensorboard_log=str(common.output_data_folder/"tensorboard"/common.args.subfolder/saved_model_filename), **hyperparams)
try:
