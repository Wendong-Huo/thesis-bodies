...
class PNSSensorAdaptor(nn.Module):
    ...
    def forward(self, obs, robot_id):
        ...
        obs = self._transform_obs(obs, robot_id)
        return obs
    
    def _transform_obs(self, obs, robot_id):
...
class CNSPNSPolicy(ActorCriticPolicy):
    def __init__(
        ...
        if common.args.cnspns_sensor_channel>0:
            self.sensor_channel = common.args.cnspns_sensor_channel
        else: # use square matrix
            self.sensor_channel = observation_space.shape[0]
        if common.args.cnspns_motor_channel>0:
            self.motor_channel = common.args.cnspns_motor_channel
        else:
            self.motor_channel = action_space.shape[0]
        ...
        if common.args.cnspns_fix_general_info:
            from common import cnspns_fgi
            self.pns_sensor_adaptor = cnspns_fgi.PNSSensorAdaptor_fix_general_info(self.sensor_channel)
        else:
