...
def get_args():
    ...
    parser.add_argument("--cnspns_sensor_channel", type=int, default=-1, help="Specify the number of sensor channels. must > fixed general info number.")
    parser.add_argument("--cnspns_motor_channel", type=int, default=-1, help="Specify the number of channels.")
    parser.add_argument("--cnspns_fix_cns", action="store_true", help="Fix the parameters in CNS, only train PNS.")
    parser.add_argument("--cnspns_fix_general_info", action="store_true", help="to utilize the prior knowledge that the order of the first 8 numbers in obs doesn't change, to help cnspns.")
    parser.add_argument("--cnspns_start_with_identity", action="store_true", help="which start with a net initialization that equivalent to no CNSPNS system.")
    parser.add_argument("--cnspns_fix_pns_bodies", type=str, default="", help="which net in pns you want to fix. e.g. --cnspns_fix_pns_bodies=399,499,599,699")
    parser.add_argument("--cnspns_relu", action="store_true", help="Instead of using linear, add a layer of non-liearity in pns.")
    ...
    args.cnspns_fix_pns_bodies_str = args.cnspns_fix_pns_bodies
    args.cnspns_fix_pns_bodies = str2array(args.cnspns_fix_pns_bodies_str)
