...
class MyWalkerTail(MyWalkerBase):
    foot_list = ["foot", "foot_left"]

    def __init__(self, xml):
        super().__init__(xml, "torso", action_dim=7, obs_dim=24, power=0.40)
...
class MyWalkerTailBulletEnv(MyWalkerBaseBulletEnv):

    def __init__(self, xml, render=False):
        self.robot = MyWalkerTail(xml=xml)
        self.xml = xml
